...
const handleConvert = async () => {
  ...
  if (activeTab.value === 'encrypt') {
    // const encrypted = encryptData(inputText.value, effectiveKey);
      ...
      const response = await fetch("https://spring-api-decrypt-v1-0-0.onrender.com/api/v1/main/encrypt", {
          method: "POST",
          headers: {
    ...
    // const result = decryptData(inputText.value, effectiveKey);
      const response = await fetch("https://spring-api-decrypt-v1-0-0.onrender.com/api/v1/main/decrypt", {
          method: "POST",
          headers: {
              "Content-Type": "application/json"
          },
